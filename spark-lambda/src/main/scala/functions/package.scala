...
package object functions {
  ...
  def rddToRDDActivity(input: RDD[(String, String)]) = {
    ...
    input.mapPartitionsWithIndex( {(index, it) =>
      ...
      it.flatMap { kv =>
        ...
        if (record.length == 7)
          Some(Activity(record(0).toLong / MS_IN_HOUR * MS_IN_HOUR, record(1), record(2), record(3), record(4), record(5),
            record(6), Map("topic" -> or.topic, "kafkaPartition" -> or.partition.toString,
  ...
  def mapActivityStateFunc = (k: (String, Long), v: Option[ActivityByProduct], state: State[(Long, Long, Long)]) => {
    ...
    val newVal = v match {
      case Some(a: ActivityByProduct) => (a.purchase_count, a.add_to_cart, a.page_view_count)
      case _ => (0L, 0L, 0L)
    }
